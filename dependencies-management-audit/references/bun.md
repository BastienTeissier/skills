...
Audit reference for repositories using **Bun** as their package manager.

Source basis: Liran Tal's npm security best-practices guide at `https://github.com/lirantal/npm-security-best-practices`, adapted for Bun's security model. Re-check upstream documentation when the latest recommendations matter.
...
Inspect: `bunfig.toml`, `package.json`

Pass when:
- `package.json` uses `trustedDependencies` to explicitly allowlist which packages may run lifecycle scripts (Bun ignores lifecycle scripts by default unless the package is in `trustedDependencies`)

Fail signals:
- `trustedDependencies` contains an overly broad list of packages
...
Inspect: `bunfig.toml`, dependency bot config

Pass when:
- `bunfig.toml` sets `minimumReleaseAge` to enforce a cooldown before new versions are installable
- Or dependency bot config (Renovate/Dependabot) enforces a minimum stabilization window

Fail signals:
- No release-age gating and no bot-level cooldown — freshly published versions are installed immediately
...
Inspect: `bun.lock` or `bun.lockb`, `package.json`

Pass when:
- `bun.lock` (or `bun.lockb`) is committed
...
Pass when:
- CI and production builds use `bun install --frozen-lockfile`

Fail signals:
- `bun install` without `--frozen-lockfile` in CI or production image builds
...
Fail signals:
- `bun update` in CI or scripts without review gates
...
Pass when:
- Public package releases use provenance-capable publishing (Bun delegates to `npm publish --provenance` or uses equivalent attestation)
...
- Bun ignores lifecycle scripts by default — the key control is whether `trustedDependencies` in `package.json` is well-scoped or overly permissive.
- Bun's lockfile format changed from binary (`bun.lockb`) to text (`bun.lock`). Check which format the repo uses.
- `bunfig.toml` supports `minimumReleaseAge` for release-age gating — this is Bun's native equivalent to pnpm's feature.
