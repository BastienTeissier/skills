# Yarn Audit Checklist

Audit reference for repositories using **Yarn** (v2+ / Berry) as their package manager.

Source basis: Liran Tal's npm security best-practices guide at `https://github.com/lirantal/npm-security-best-practices`, adapted for Yarn's security model. Re-check upstream documentation when the latest recommendations matter.
...
Inspect: `.yarnrc.yml`, `package.json`

Pass when:
- `.yarnrc.yml` sets `enableScripts: false` globally and allowlists trusted packages via `dependenciesMeta[pkg].built: true`
- Or lifecycle scripts are otherwise restricted

Fail signals:
- Unrestricted install with all lifecycle scripts running freely
...
Inspect: `.yarnrc.yml`, dependency bot config

Pass when:
- `.yarnrc.yml` sets `npmMinimalAgeGate` to enforce a minimum release age before installation
- Or dependency bot config (Renovate/Dependabot) enforces a minimum stabilization window

Fail signals:
- No age gate and no bot-level cooldown — freshly published versions are installed immediately
...
Inspect: `yarn.lock`, `.yarnrc.yml`, `package.json`

Pass when:
- `yarn.lock` is committed
- Lockfile is validated with `lockfile-lint` or an equivalent policy
...
Pass when:
- CI and production builds use `yarn install --immutable` (Yarn Berry) or `yarn install --frozen-lockfile` (Yarn Classic)

Fail signals:
- `yarn install` without immutable/frozen-lockfile flags in CI or production image builds
...
Fail signals:
- `yarn up` or `yarn upgrade` in CI or scripts without review gates
...
Pass when:
- Public package releases use provenance-capable publishing (Yarn delegates to `npm publish --provenance` or uses equivalent attestation)
...
- Yarn Berry (v2+) and Yarn Classic (v1) have different CLI flags. Check `packageManager` in `package.json` or `.yarnrc.yml` presence to determine the version.
- Yarn Berry's `enableScripts: false` + `dependenciesMeta` allowlisting is the primary mechanism for constraining install-time execution.
